...
import { Conductor } from '../../shared/models';
...
export class ConductoresComponent implements OnInit {
  ...
  cargarDatos() { this.cargando = true; this.error = null; this.conductoresApi.getConductores().subscribe({ next: (r) => { this.conductores = r.data || []; this.cargando = false; }, error: (e) => { this.error = 'Error: ' + e.message; this.cargando = false; } }); }
  ...
  guardar() {
    try {
      ...
      if (this.editando) {
        ...
        this.conductoresApi.updateConductor(id, data).subscribe({
        ...
        this.conductoresApi.createConductor(this.form as Conductor).subscribe({
          next: () => { this.cargarDatos(); this.cerrarModal(); },
  ...
  ejecutarEliminar() { if (!this.eliminarId) return; this.cargando = true; this.conductoresApi.deleteConductor(this.eliminarId).subscribe({ next: () => { this.cargarDatos(); this.modalEliminar = false; this.eliminarId = null; }, error: (e) => { alert('Error: ' + e.message); this.cargando = false; } }); }
