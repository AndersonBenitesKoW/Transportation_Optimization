...
import { AlertasApi, ChatApi, FlotaApi, VehiculosApi, ViajesApi } from '../../core/api';
...
export class ConductorComponent implements OnInit, OnDestroy {
  ...
  cargarDesdeCRUD() {
    ...
    if (!ref) return;
    this.vehiculosApi.getVehiculo(ref).subscribe({
      next: (r: any) => {
        const v = r.data;
        if (v) {
          this.miCamion = {
  ...
  enviarConsulta() {
    ...
    this.chatApi.enviarMensajeChat(pregunta, this.usuarioActual.rol, this.usuarioActual.ref).subscribe({
      next: (res: any) => {
