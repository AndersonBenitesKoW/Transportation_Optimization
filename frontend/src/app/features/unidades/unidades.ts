...
import { ConductoresApi, VehiculosApi } from '../../core/api';
import { Componente, Conductor, Vehiculo } from '../../shared/models';
...
export class UnidadesComponent implements OnInit {
  ...
  cargarDatos() {
    ...
    this.vehiculosApi.getVehiculos().subscribe({ next: (r) => { this.unidades = r.data || []; this.cargando = false; }, error: (e) => { this.error = 'Error: ' + e.message; this.cargando = false; } });
    this.conductoresApi.getConductores().subscribe({ next: (r) => this.conductores = r.data || [] });
  }
  ...
  cargarComponentes(id: string): Promise<void> {
    return new Promise((resolve) => {
      this.resetComponentes();
      this.vehiculosApi.getComponentes(id).subscribe({
        next: (r) => {
  ...
  guardarTodo() {
    ...
    if (this.modo === 'componentes') {
      ...
      const guardarVehiculo = this.modo === 'editar'
        ? this.vehiculosApi.updateVehiculo(this.form.id_vehiculo!, (({ id_vehiculo, ...rest }) => rest)(this.form))
        : this.vehiculosApi.createVehiculo(this.form as Vehiculo);
  ...
  private guardarComponentesYSalir() {
    const eliminados$ = this.componentesEliminados.map(nombre =>
      this.vehiculosApi.deleteComponente(this.form.id_vehiculo!, nombre)
    );
    ...
    const finalizar = () => {
      ...
      this.vehiculosApi.updateComponentes(this.form.id_vehiculo!, compsPayload).subscribe({
  ...
  ejecutarEliminar() { if (!this.eliminarId) return; this.cargando = true; this.vehiculosApi.deleteVehiculo(this.eliminarId).subscribe({ next: () => { this.cargarDatos(); this.modalEliminar = false; this.eliminarId = null; }, error: (e) => { alert('Error: ' + e.message); this.cargando = false; } }); }
